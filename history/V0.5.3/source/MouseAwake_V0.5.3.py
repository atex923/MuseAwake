...
class FlameOverlay:
    ...
    def _build_coverage_plan(self) -> None:
        ...
        # 保留「半格錯位」效果。
        jitter = self.coverage_spacing * 0.12

        for row in range(-1, rows + 2):
            for column in range(-1, columns + 2):
                center_x = (
                    column * self.coverage_spacing
                    + self.coverage_spacing / 2
                    + self.offset_x
                )
                center_y = (
                    row * self.coverage_spacing
                    + self.coverage_spacing / 2
                    + self.offset_y
                )
                ...
                # 允許中心點稍微超出桌面，
                # 這樣邊緣仍會被半個火焰覆蓋。
                margin = self.coverage_spacing

                if (
                    -margin <= center_x
                    <= self.virtual_width + margin
                    and
                    -margin <= center_y
                    <= self.virtual_height + margin
                ):
                    plan.append(
                        (center_x, center_y)
                    )
    ...
    def round_status_text(self) -> str:
        ...
        return (
            f"位移輪 {self.shift_round_index}/5 "
